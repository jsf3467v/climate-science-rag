...
This serves as a guide to run the model. `app.py` answers through
`synthesize.context()` and `answer()`, the same HyDE-expanded BM25, Claude rerank,
and grounded synthesis path the evaluation scored, so the demo and the reported
...
Two prebuilt artifacts are referenced by paths relative to the repo root.
...
UI. The Space card takes an `emoji:` field. It is omitted from the front matter on
purpose, so add one in the settings UI if you want a thumbnail.
...
- The BM25 index plus chunks load to about 2 GB resident (measured; the about
  325 MB on disk expands as Python objects), and request handling adds little on
  top. The free CPU Basic tier (2 vCPU, 16 GB RAM) has ample headroom.
